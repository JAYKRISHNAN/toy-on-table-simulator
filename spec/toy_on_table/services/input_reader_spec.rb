...
describe ToyOnTable::Services::InputReader do
  describe '.initialize' do
    it 'sets file_path' do
      service = ToyOnTable::Services::InputReader.new('my_pat/my_file')

      expect(service.file_path).to eq 'my_pat/my_file'
    end
  end

  describe '.read' do
    it 'reads file line by line and returns a list of commands with given input lines' do
      file_path = 'test.txt'
      ...
      expect(ToyOnTable::Models::Command).to receive(:new).with("TEST COMMAND 1\n", 0).and_call_original
      expect(ToyOnTable::Models::Command).to receive(:new).with("TEST COMMAND 2\n", 1).and_call_original
